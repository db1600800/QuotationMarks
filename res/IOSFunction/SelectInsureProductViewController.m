...
#import "SelectInsureProductViewController.h"
...
#import "SelectInsureProductTableViewCell.h"
#import "RespondParam8056130.h"

//注入table功能
NSString *SelectInsureProductCellIdentifier = @"SelectInsureProductTableViewCell";
NSString *SelectInsureProductCellHeadIdentifier = @"SelectInsureProductTableViewCellHead";

@implementation SelectInsureProductViewController
...
- (void)viewDidLoad
{
    ...
    UINib *cellNib = [UINib nibWithNibName:@"SelectInsureProductTableViewCell" bundle:nil];
    [tableView registerNib:cellNib forCellReuseIdentifier:SelectInsureProductCellIdentifier];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    {
    RespondParam8056130 *item1 = rows[index];
   
    return  [item1.products count];
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {
        SelectInsureProductTableViewCell *cell = (SelectInsureProductTableViewCell*)[self.tableView dequeueReusableCellWithIdentifier:SelectInsureProductCellIdentifier];
        if (!cell)
        {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"SelectInsureProductTableViewCell" owner:self options:nil] lastObject];
        }
        
      
        RespondParam8056130 *product;
        RespondParam8056130 *item1 = rows[index];
        product= item1.products[indexPath.row];
                
        
        
        
        [cell.whichCheckBox setSelected:product.productSelect];
        //选择a
        ...
        [cell.whichCheckBoxCover setText:product.D44_70_MATTERNAME];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *reuseIdentifier = SelectInsureProductCellIdentifier;
    SelectInsureProductTableViewCell *cell= [self.cacheCells objectForKey:reuseIdentifier];
    if (!cell) {
        cell=[self.tableView dequeueReusableCellWithIdentifier:SelectInsureProductCellIdentifier];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    for(int j=0;j<[item1.products count];j++)
    {
        if (j==indexPath.row) {
            product= item1.products[j];
            product.productSelect=true;
        }else
        {product= item1.products[j];
            product.productSelect=false;
